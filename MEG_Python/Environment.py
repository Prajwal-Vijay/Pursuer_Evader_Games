...
class Environment:
    def __init__(self, N, n, v, u, r, pursuer_position=None,
                 evader_positions=None, target_position=None):
        ...
        if evader_positions is not None:
            for i in range(n):
                self.evaders.append(
                    Evader.Evader(evader_positions[:, i:i + 1], u[i], i))
        else:
            for i in range(n):
                self.evaders.append(
                    Evader.Evader(np.random.rand(N, 1), u[i], i))
    ...
    def barrier(self, evaders):
        ...
        for i, evader in enumerate(evaders):
            evader_dist_sq = np.linalg.norm(
                self.target_position - evader.position)**2
            pursuer_dist_sq = np.linalg.norm(
                self.target_position - self.pursuer.position)**2
            barriers[i] = (evader_dist_sq -
                           self.alpha[evader.index]**2 * pursuer_dist_sq)
    ...
    def plot_current_positions(self):
        ...
        for evader in self.evaders:
            plt.plot(evader.position[0, 0], evader.position[1, 0], 'bo',
                     markersize=10,
                     label='Evader' if evader.index == 0 else "")
        # Plot target
        plt.plot(self.target_position[0, 0], self.target_position[1, 0], 'go',
                 markersize=10, label='Target')
        # Plot pursuer
        plt.plot(self.pursuer.position[0, 0], self.pursuer.position[1, 0], 'ro',
                 markersize=10, label='Pursuer')
    ...
    def return_evader_velocities(self, evaders):
        ...
        for i, evader in enumerate(self.evaders):
            ...
            if not self.captured_evaders[i]:
                ...
                # Uses the position of pursuer, target, and ratio of speed
                # to find the velocity.
                velocity, _ = evader.heading_velocity(self.pursuer.position,
    ...
    def update_termination(self):
        ...
        for i, evader in enumerate(self.evaders):
            if (np.linalg.norm(self.pursuer.position - evader.position) <
    ...
    def step(self, objective_function):
        ...
        for i, evader in enumerate(self.evaders):
            if not self.captured_evaders[i]:
                evader_at_target = (
                    np.linalg.norm(evader.position - self.target_position) <
                    self.capture_tolerance)
                pursuer_at_target = (
                    np.linalg.norm(self.pursuer.position -
                                   self.target_position) >
                    2 * self.capture_tolerance)

                if evader_at_target and pursuer_at_target:
                    print(f'{evader.name} reached the target')
                    return True

        # All active evaders are done, stop game
        active_evaders = [evader for i, evader in enumerate(self.evaders)
                          if not self.captured_evaders[i]]
        ...
        # This is the complex mathematics function that calculates
        # pursuer velocity
        pursuer_velocity, _ = self.pursuer.heading_velocity(
            evader_positions, self.target_position, self.timestep, win,
            objective_function)
        evader_velocities = self.return_evader_velocities(self.evaders)

        # Update positions
        self.pursuer.update_pos(
            self.pursuer.position + self.timestep * pursuer_velocity)
        for i, evader in enumerate(self.evaders):
            if not self.captured_evaders[i]:
                evader.update_pos(
                    evader.position + self.timestep *
                    evader_velocities[:, i:i + 1])
        return False
...
#            t += 1
#            self.plot_trajectories(pursuer_positions_traj,
#                                   evader_positions_traj)
#            if t % 100 == 0:
#                print(f"Step {t}")
#
...
#        return win, pursuer_positions_traj, evader_positions_traj
    ...
    def obtain_trajectories(self, objective_function, max_steps=10000):
        ...
        evader_plots = [
            ax.plot([], [], 'bo', markersize=10,
                    label='Evader' if i == 0 else "")[0]
            for i in range(self.evader_numbers)
        ]
        target_plot, = ax.plot(self.target_position[0, 0],
                               self.target_position[1, 0],
                               'go', markersize=10, label='Target')
        pursuer_traj_line, = ax.plot([], [], 'r-', linewidth=2,
                                     label='Pursuer Trajectory')
        evader_traj_lines = [
            ax.plot([], [], 'b-', linewidth=2,
                    label=f'Evader {i} Trajectory' if i == 0 else "")[0]
            for i in range(self.evader_numbers)
        ]
        ...
        while not done and t < max_steps:
            ...
            # Update plot
            pursuer_plot.set_data([self.pursuer.position[0, 0]],
                                  [self.pursuer.position[1, 0]])
            for i, evader in enumerate(self.evaders):
                if not self.captured_evaders[i]:
                    evader_plots[i].set_data([evader.position[0, 0]],
                                             [evader.position[1, 0]])
    ...
    """ NOT BEING USED
    def plot_trajectories(self, pursuer_positions_traj,
                          evader_positions_traj):
        ...
        plt.plot(pursuer_x, pursuer_y, 'r-', label='Pursuer trajectory',
                 linewidth=2)
        ...
        for i, traj in enumerate(evader_positions_traj):
            ...
            plt.plot(evader_x, evader_y, 'b-',
                     label=f'Evader {i} trajectory' if i == 0 else "",
                     linewidth=2)
