...
class Pursuer:
    def __init__(self, init_pos, speed):
        ...
        if np.array(init_pos).ndim == 1:
            self.position = np.array(init_pos).reshape(-1, 1)
        else:
    ...
    def optimal_headings_Ei(self, evader_position, target_position):
        ...
        if self.position[0, 0] != evader_position[0, 0]:
            m = ((evader_position[1, 0] - self.position[1, 0]) /
                 (evader_position[0, 0] - self.position[0, 0]))
        else:
            m = float('inf')
        ...
        psi_star = np.arctan2(yI - evader_position[1, 0],
                              xI - evader_position[0, 0])
        theta_star = np.arctan2(yI - self.position[1, 0],
                                xI - self.position[0, 0])
    ...
    def objective_Ei(self, evader_position, target_position, r, theta):
        ...
        psi_star, _, m, xm, ym = self.optimal_headings_Ei(evader_position,
                                                          target_position)
        evader_position = np.array(evader_position).reshape(-1, 1)
        target_position = np.array(target_position).reshape(-1, 1)
        ...
        if m != float('inf'):
            ...
            Ty = abs(yt - m * xt - evader_position[1, 0] +
                     m * evader_position[0, 0]) / np.sqrt(1 + m**2)
        else:
            Tx = abs(xt - xm)
        ...
        if k > 0:
            term_sqrt = np.sqrt(Ty**2 + k**2)
            term_cos = (-1 - np.cos(theta + psi_star - 2 * delta))
            d_i = Tx + (r / (2 * k)) * term_sqrt * term_cos
        else:
            d_i = Tx
    ...
    def concave_domain(self, evader_positions, target_position):
        ...
        for i in range(n):
            _, theta_star = self.optimal_headings_Ei(
                evader_positions[:, i:i + 1], target_position)
    ...
    def objective_fun(self, evader_positions, target_position, r, theta):
        ...
        cost = 0
        for i in range(n):
            obj_val = self.objective_Ei(
                evader_positions[:, i:i + 1], target_position, r, theta)
            if obj_val != 0:
                cost += 1 / obj_val
        return cost

    def heading_direction_heuristic(self, evader_positions,
                                    target_position, r):
        ...
        for i in range(n):
            _, theta_star[i], _, _, _ = self.optimal_headings_Ei(
                evader_positions[:, i:i + 1], target_position)

        for i in range(n):
            p_values[i] = self.objective_Ei(
                evader_positions[:, i:i + 1], target_position, r,
                theta_star[i])
    ...
    def heading_direction_closest(self, evader_positions):
        ...
        for i in range(n):
            distances[i] = np.linalg.norm(self.position -
                                          evader_positions[:, i:i + 1])

        closest_idx = np.argmin(distances)
        theta = np.arctan2(evader_positions[1, closest_idx] -
                           self.position[1, 0],
                           evader_positions[0, closest_idx] -
                           self.position[0, 0])
        return theta

    def heading_velocity(self, evader_positions, target_position,
                         r, win, objective_function):
        """Calculate heading velocity."""
        if win:
            if objective_function == 'heuristic':
                theta = self.heading_direction_heuristic(
                    evader_positions, target_position, r)
            elif objective_function == 'closest':
                theta = self.heading_direction_closest(evader_positions)
            else:
                print(f'Unknown objective function: {objective_function}. '
                      'Using heuristic.')
                theta = self.heading_direction_heuristic(
                    evader_positions, target_position, r)

            velocity = self.speed * np.array([[np.cos(theta)],
                                              [np.sin(theta)]])
        else:
            target_position = np.array(target_position).reshape(-1, 1)
