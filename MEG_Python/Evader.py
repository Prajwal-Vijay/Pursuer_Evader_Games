    ...
    def heading_velocity(self, pursuer_position, target_position, win, alpha):
        ...
        if win and alpha == 1:
            ...
            if self.position[0, 0] != pursuer_position[0, 0]:
                m = ((self.position[1, 0] - pursuer_position[1, 0]) /
                     (self.position[0, 0] - pursuer_position[0, 0]))
                x_intercept = (
                    (m * (yc - target_position[1, 0]) + xc + m**2 *
                     target_position[0, 0]) / (1 + m**2))
                y_intercept = (target_position[1, 0] +
                               m * (x_intercept - target_position[0, 0]))
            else:
                x_intercept = xc
                y_intercept = target_position[1, 0]

            velocity = np.array([[x_intercept - self.position[0, 0]],
                                 [y_intercept - self.position[1, 0]]])

        elif win and alpha < 1:
            xc = ((self.position[0, 0] - alpha**2 * pursuer_position[0, 0]) /
                  (1 - alpha**2))
            yc = ((self.position[1, 0] - alpha**2 * pursuer_position[1, 0]) /
                  (1 - alpha**2))
            rc = ((alpha / (1 - alpha**2)) *
                  np.linalg.norm(pursuer_position - self.position))
